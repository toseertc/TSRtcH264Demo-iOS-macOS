...
@interface TSVideoCapturer () <AVCaptureVideoDataOutputSampleBufferDelegate>
...
@property (nonatomic, strong) TSVideoDropFrameManager *dropFrameManager;

@end


@implementation TSVideoCapturer
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        _dropFrameManager = [[TSVideoDropFrameManager alloc] init];
...
//默认设备
#if TARGET_OS_IOS
            self.currentDevice = [TSVideoCapturerUtil videoCaptureDeviceWithPosition:AVCaptureDevicePositionFront];
...
- (void)configureSession
{
    ...
    NSInteger dstRate;
    [TSVideoCapturerUtil setCaptureFrameRate:self.videoConfig.frameRate dstSize:self.videoConfig.dimension forDevice:captureDevice resultFrameRate:&dstRate];
...
- (int)start
{
    if (self.session.isRunning) {
        if ([self.delegate respondsToSelector:@selector(videoCapturer:didStartWithReason:)]) {
            [self.delegate videoCapturer:self didStartWithReason:TSVCamStartReasonUserTrigger];
        }
        return 0;
    }
    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        if (granted) {
            ...
            if ([self.delegate respondsToSelector:@selector(videoCapturer:didStopWithReason:)]) {
                [self.delegate videoCapturer:self didStopWithReason:TSVCamStopReasonNoPermission];
            }
        }
    }];
...
- (void)startSession
{
    dispatch_async(self.sessionQueue, ^{
        if (self.setupResult == AVCamSetupResultSuccess) {
            ...
            if ([self.delegate respondsToSelector:@selector(videoCapturer:didStopWithReason:)]) {
                [self.delegate videoCapturer:self didStopWithReason:TSVCamStopReasonSetupError];
            }
        }
    });
}
...
- (int)stopWithCallback:(void (^)(void))callback
{
    if (!self.session.isRunning) {
        if ([self.delegate respondsToSelector:@selector(videoCapturer:didStopWithReason:)]) {
            [self.delegate videoCapturer:self didStopWithReason:TSVCamStopReasonUserTrigger];
        }
        if (callback) {
            callback();
        }
...
- (void)swapFrontAndBackCameras
{
    dispatch_async(self.sessionQueue, ^{
        ...
        AVCaptureDevice *nextCaptureDevice = [TSVideoCapturerUtil videoCaptureDeviceWithPosition:preferredPosition];
...
#endif
        NSInteger dstRate;
        [TSVideoCapturerUtil setCaptureFrameRate:videoConfig.frameRate
...
- (void)setCurrentInputDevice:(nonnull AVCaptureDevice *)currentInputDevice
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        NSInteger dstRate;
        [TSVideoCapturerUtil setCaptureFrameRate:self.videoConfig.frameRate dstSize:self.videoConfig.dimension forDevice:currentInputDevice resultFrameRate:&dstRate];
        NSLog(@"%s, fps = %ld", __func__, (long)dstRate);
...
{
    if ([self.delegate respondsToSelector:@selector(videoCapturer:didStopWithReason:)]) {
        [self.delegate videoCapturer:self didStopWithReason:TSVCamStopReasonSystemInterrupt];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(videoCapturer:didStartWithReason:)]) {
        [self.delegate videoCapturer:self didStartWithReason:TSVCamStartReasonSystemInterruptEnd];
    }
}
...
- (void)sessionRuntimeError:(NSNotification *)notification
{
    ...
    if (error.code == AVErrorMediaServicesWereReset) {
        ...
        if ([self.delegate respondsToSelector:@selector(videoCapturer:didStopWithReason:)]) {
            [self.delegate videoCapturer:self didStopWithReason:TSVCamStopReasonSystemError];
        }
    }
}
