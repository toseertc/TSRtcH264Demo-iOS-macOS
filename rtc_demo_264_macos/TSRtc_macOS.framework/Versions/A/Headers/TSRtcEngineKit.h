...
/**
 设置SDK 输出的日志文件。
 如需要调用本方法，请在调用`sharedEngineWithConfig`后立即调用，否则可能造成输出日志不完整。
 @param filePath 日志文件的完整路径。请确保指定的目录存在而且可写。你可通过该参数修改日志文件名。
 @return 0 调用成功，<0 调用失败。
...
/**
 设置日志输出等级。
 设置 输出日志输出等级。不同的输出等级可以单独或组合使用。日志级别顺序依次为 OFF、CRITICAL、ERROR、WARNING、INFO 和 DEBUG。选择一个级别，你就可以看到在该级别之前所有级别的日志信息。
